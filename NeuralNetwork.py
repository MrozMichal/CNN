...
class NeuralNetwork:
    ...
    def initialize_network(self):
        ...
        self.Wages = [random.uniform(-self.random_wages, self.random_wages) for i in range(self.n_inputs + 1)]
        self.hidden_layer = [
            {'weights': [random.uniform(-self.random_wages, self.random_wages) for i in range(self.n_inputs + 1)],
             'prev_weights': [0 for i in range(self.n_inputs + 1)]} for i in range(self.n_hidden)]
        self.network1.append(self.hidden_layer)
        self.output_layer = [
            {'weights': [random.uniform(-self.random_wages, self.random_wages) for i in range(self.n_hidden + 1)],
             'prev_weights': [0 for i in range(self.n_hidden + 1)]} for i in range(self.n_outputs)]
        self.network1.append(self.output_layer)
        return self.network1, self.Wages
